...
def ReadSingleXDSIntegrateHKL(in_ascii):
	"""
	Takes INTEGRATE.HKL file generated by XDS INTEGRATE
	Returns a dataframe with Fhkl and hkl indices 
	"""
	asci = pd.read_csv(in_ascii,header=None, sep='\s+', engine='python', \
		names=['h','k','l','I','sigI','xd','yd','zd','rlp','peak','cor','maxc',\
		'xobs','yobs','zobs', 'alf0','bet0','alf1','bet1','psi','iseg'],\
		skiprows=32,skipfooter=1)

	asci['hkl'] = \
	asci.h.apply(lambda x:str(x)) + ' ' + \
	asci.k.apply(lambda x:str(x)) + ' ' + \
	...
	# Drop unobserved peaks which marked with 0 in xobs, yobs and zobs columns
	# see xds wiki for further info
	asci = asci[(asci.xobs!=0) & (asci.yobs!=0) & (asci.zobs!=0)]
	asci.sort_values(by='res',ascending=False,inplace=True)
...
class XDSIntegrate:
	...
	def __init__(self,name_template,space_group):
		...
		if glob(join(self.dir_name,'*desc.txt')) == []:
			...
			if not os.path.isfile(pdb_file):
				print "MyError: Place the original pdb file in the folder!"
			else:
	...
	def ProcessStills(self,minpk=75):
		...
		Saves INTEGRATE.LP, INTEGRATE.HKL, CORRECT.LP and XDS_ASCII.HKL files from each frame
		desc file and the original pdb file must be under the folder for this to work
		"""
		cwd = os.getcwd()
		os.chdir(self.dir_name)

		os.system('rm ' + join(self.dir_name,'INTEGRATE_*.HKL')) # clear previous results if any
		os.system('rm ' + join(self.dir_name,'INTEGRATE_*.LP'))
		os.system('rm ' + join(self.dir_name,'XDS_ASCII_*.HKL'))
		...
		for img in img_list:
			...
			if not os.path.isfile(join(self.dir_name,'XDS.INP')):
				print "MyError: Place XDS.INP file in the folder!"
				break
			...
			subprocess.call(['xds'])
			subprocess.call(['mv', 'INTEGRATE.HKL', 'INTEGRATE_%s.HKL' %fnumber])
			subprocess.call(['mv', 'INTEGRATE.LP', 'INTEGRATE_%s.LP' %fnumber])
	...
	def ReadAllPeaks(self):
		"""
		Reads peak intensities from multi INTEGRATE.HKL files and merges them into a single df
		"""
		cwd = os.getcwd()
		os.chdir(self.dir_name)
		asci_list = [x for x in os.listdir(self.dir_name) if x.startswith('INTEGRATE_') and x.endswith('.HKL')]
		asci_byframe = pd.DataFrame()
		for asci_file in asci_list:
			fnumber = int(asci_file.split('_')[1][0:3])
			asci = ReadSingleXDSIntegrateHKL(asci_file)
			...
			asci.drop(columns=['h','k','l','sigI','rlp','maxc','xobs','yobs','zobs',\
				'alf0','bet0','alf1','bet1','psi','iseg'],inplace=True)
			asci.rename(columns={'I':'I_'+str(fnumber), 'xd':'xd_'+str(fnumber), 'yd':'yd_'+str(fnumber),\
	...
	def ReadMosCellBfac(self):
		"""
		Read mosaicity and unit cell from INTEGRATE.LP
		Read Wilson-B from CORRECT.LP
		Returns dataframe
		"""
		cwd = os.getcwd()
		os.chdir(self.dir_name)

		integrateLP_list = [x for x in os.listdir(self.dir_name) if x.startswith('INTEGRATE_') and x.endswith('.LP')]
		correctLP_list = [x for x in os.listdir(self.dir_name) if x.startswith('CORRECT_') and x.endswith('.LP')]
		results_byframe = pd.DataFrame(columns=['mosaicity','cellA','cellB','cellC','wilsonB'])

		# Read mosaicity and unit cell from INTEGRATE.LP 
		for integrateLP_file in integrateLP_list:
			for line in fileinput.FileInput(integrateLP_file):
				fnumber = int(integrateLP_file.split('_')[1][0:3])
		...
		# Read wilson-B from CORRECT.LP 
		for correctLP_file in correctLP_list:
			for line in fileinput.FileInput(correctLP_file):
				fnumber = int(correctLP_file.split('_')[1][0:3])
	...
	def PlotMosCellBfac(self,save_img=True):
		...
		# Mosaicity
		axs[0,0].scatter(mcb.index,mcb.mosaicity,label='XDS (Integrate)',facecolor='tab:blue',s=30)
		...
		# B-factor
		axs[0,1].scatter(mcb.index,mcb.wilsonB,label='XDS (Correct)',facecolor='tab:orange',s=30)
		...
		# Cell-A
		axs[1,0].scatter(mcb.index,mcb.cellA,label='XDS (Integrate)',facecolor='tab:green',s=30)
		...
		# Cell-B
		axs[1,1].scatter(mcb.index,mcb.cellB,label='XDS (Integrate)',facecolor='tab:red',s=30)
		...
		# Cell-C
		axs[1,2].scatter(mcb.index,mcb.cellC,label='XDS (Integrate)',facecolor='tab:purple',s=30)
		...
		plt.tight_layout()
		if save_img:
			fig.savefig( join(self.dir_name,"fig_XDS(Integrate)_MosCellBfac.png"), dpi=200 )
		plt.show()
		self.mcb = mcb
		return mcb
	...
	def PlotPeakIntensities(self,asci_byframe,first_N_peaks=None,save_img=True):
		...
		ax.set_ylabel('XDS (Integrate) Peak Intensity\n(indexed-integrated $\\times 10^%i$)' \
			%zeros,fontsize='x-large')
		...
		plt.tight_layout()
		if save_img:
			fig.savefig( join(self.dir_name,"fig_XDS(Integrate)_PeakIntensities.png"), dpi=200 )
		plt.show()
	...
	def PlotIntegratedIntensities(self,asci_byframe,scale='log',save_img=True):
		...
		if scale == 'linear':	
			...
			ax.set_ylabel('XDS (Integrate) Integrated Intensity\n(indexed-integrated $\\times 10^%i$)' \
				%zeros,fontsize='x-large')
			ax.yaxis.set_minor_locator(AutoMinorLocator())		
		elif scale == 'log':
			ax.plot(frame_numbers,int_intensities,marker='o',lw=2)
			ax.set_yscale('log')
			ax.set_ylabel('XDS (Integrate) Integrated Intensity \n(indexed-integrated)',fontsize='x-large')
		...
		plt.tight_layout()
		if save_img:
			fig.savefig( join(self.dir_name,"fig_XDS(Integrate)_IntIntensities.png"), dpi=200 )
		plt.show()
	...
	def PlotShellIntensities(self,asci_byframe,N_shells=10,npeak_ratio=1,normalized=True,save_img=True):
		...
		if save_img:
			fig.savefig(join(self.dir_name,"fig_XDS(Integrate)_Shellintensities.png"),dpi=200)
		plt.show()
		self.shells = df_shells
		return df_shells
	...
	def PlotDhalfvsResolution(self,asci_byframe,N_shells=10,npeak_ratio=1,\
		...
		ax.set_ylabel('XDS (Integrate) Half-dose (arbitrary)',fontsize='x-large')
		...
		plt.tight_layout()
		if save_img:
			fig.savefig(join(self.dir_name,"fig_XDS(Integrate)_DhalfvsRes.png"),dpi=200)
		plt.show()
		return df_shells
